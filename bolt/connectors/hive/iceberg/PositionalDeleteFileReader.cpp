...
#include "bolt/connectors/hive/iceberg/PositionalDeleteFileReader.h"
...
PositionalDeleteFileReader::PositionalDeleteFileReader(
  ...
  BOLT_CHECK(deleteFile_.content == FileContent::kPositionalDeletes);
  BOLT_CHECK(deleteFile_.recordCount);
  ...
  options.fileSize = deleteFile_.fileSizeInBytes;
  std::vector<int> columnCacheBlackList;
  auto deleteFileHandle =
      fileHandleFactory_->generate(deleteFile_.filePath, options).second;
  auto deleteFileInput = hive::createBufferedInput(
      *deleteFileHandle,
      deleteReaderOpts,
      connectorQueryCtx_,
      ioStats_,
...
bool PositionalDeleteFileReader::readFinishedForBatch(
    int64_t rowNumberUpperBound) {
  BOLT_CHECK_NOT_NULL(deletePositionsOutput_);
